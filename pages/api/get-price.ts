...
export default async function handler(
  ...
  if (fresh && typeof CACHE.price === "number") {
    return res
      .status(200)
      .json({ ok: true, ethPrice: CACHE.price, cached: true });
  }
  ...
  try {
    ...
    if (typeof CACHE.price === "number") {
      return res
        .status(200)
        .json({ ok: true, ethPrice: CACHE.price, cached: true, stale: true });
    }
    console.log("here");
    return res
      .status(500)
      .json({ ok: false, error: e?.message ?? "Failed to fetch price" });
  }
}
